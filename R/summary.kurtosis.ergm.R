#' Summarizing Kurtosis ERGM Model Fits
#' 
#' \code{\link[base]{summary}} method for [`kurtosis.ergm`] fits.
#' 
#' \code{\link{summary.kurtosis.ergm}} tries to be smart about formatting the
#' coefficients, standard errors, etc.
#' 
#' @aliases print.summary.kurtosis.ergm
#' @param object an object of class \code{"kurtosis.ergm"}, usually, a result
#'   of a call to \code{\link{ergm.kurtosis}}.
...
#' 
#' @export
summary.kurtosis.ergm <- function (object, ..., 
                          ...
                          total.variation=TRUE)
{
  ...
  M4.names <- cnames.all[grep("offset(M4(",cnames.all,fixed=TRUE)]
  if(length(M4.names)>0){
    trim.var.names <- 11
  }else{
    M4.names <- cnames.all[grep("M4(",cnames.all,fixed=TRUE)]
    trim.var.names <- 4
  }
  if(length(M4.names)>0){
    var.names <- sapply(strsplit(substr(M4.names,start=trim.var.names,stop=1000),")",fixed=TRUE),
                        function(x){x[1]})
    cnames <- match(var.names,cnames.all)
  }else{
    cnames <- grep("Var(",cnames.all,fixed=TRUE,invert=TRUE)
  }
...
# null.lik<-try(logLikNull(object,...), silent=TRUE)
  ...
  ans$asycov <- asycov
  ans$asyse <- asyse
  class(ans) <- "summary.kurtosis.ergm"
  ans
}
