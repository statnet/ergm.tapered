...
#' @param MCMLE.kurtosis.equality Logical: If TRUE, constrain the kurtosis.
...
control.ergm.tapered<-function(
  ...
  if(is.null(control["MCMC.esteq.exclude.statistics"])){
    control["MCMC.esteq.exclude.statistics"] <- switch(control$MCMLE.metric,
      Kpenalty="Taper_Penalty",
      kurtosis="Var(",
      NULL)
  }
...
#STATIC_TAPERING_CONTROLS <- c("MCMLE.kurtosis.prior", "MCMLE.kurtosis.location", "MCMLE.kurtosis.scale")
