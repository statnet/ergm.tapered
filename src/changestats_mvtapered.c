...
D_CHANGESTAT_FN(d_var_term){
  GET_STORAGE(StoreModelAndStats, storage);
  Model *m = storage->m;

  // TODO: cache the result for use in u_var_term.
  ...
  for(unsigned int k=0; k<m->n_stats; k++){
    ...
    CHANGE_STAT[k] = INPUT_PARAM[k]*(new_diff*new_diff - storage->stats[k]*storage->stats[k]);
  }
}

U_CHANGESTAT_FN(u_var_term){
  GET_STORAGE(StoreModelAndStats, storage);
  Model *m = storage->m;
