...
D_CHANGESTAT_FN(d_kurt_term){
  GET_STORAGE(StoreModelAndStats, storage);
  Model *m = storage->m;

  // TODO: cache the result for use in u_kurt_term.
  ...
  for(unsigned int k=0; k<nstat; k++){
    ...
    CHANGE_STAT[k      ] = INPUT_PARAM[k      ]*(tmp1 - tmp2);
    CHANGE_STAT[k+nstat] = INPUT_PARAM[k+nstat]*(tmp1*tmp1 - tmp2*tmp2);
